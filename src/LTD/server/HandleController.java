...
import LTD.real.CloneCharacter;
import LTD.real.Player;
import LTD.real.UseSkill;
import LTD.real.Language;
import LTD.real.Char;
import LTD.real.Map;
import LTD.real.Friend;
import LTD.real.ClanManager;
import LTD.real.Party;
import LTD.real.ItemSell;
import LTD.real.PartyPlease;
import LTD.real.UseItem;
import LTD.real.Item;
import LTD.io.Message;
import LTD.io.SQLManager;
import LTD.io.Util;
import LTD.thiendiabang.ThienDiaBang;
        ...
        try {
            if (player != null && player.c != null && m != null && m.reader().available() > 0) {
                ...
                if ("xu".equals(chat) && player.id == 1) {
                    ...
                    player.conn.sendMessageLog("Cấp hiền nhân : " + player.c.lvhnt + "\nExp hiền nhân : " + player.c.exphnt + "\nCấp bht : " + player.c.lvbht + "\nExp bht : " + player.c.expbht + "\nDùng sách tiềm năng trung cấp còn lại : " + player.c.tntrung + "/4 lần"+ "\nDùng sách kỹ năng trung cấp còn lại: " + player.c.kntrung + "/4 lần");
                ...
                } else if (chat.equals("a") && player.role == 9999) {
                    Service.ShowAdmin(player);
                } 
                else if (chat.equals("b") && player.role == 9999) {
                    Item itemup = ItemTemplate.itemDefault(386);
                    Item itemup1 = ItemTemplate.itemDefault(387);
                    Item itemup2 = ItemTemplate.itemDefault(388);
                    Item itemup3 = ItemTemplate.itemDefault(393);
                    Item itemup4 = ItemTemplate.itemDefault(394);
                    Item itemup5 = ItemTemplate.itemDefault(395);
                         itemup.quantity = 10000;
                         itemup1.quantity = 10000;
                         itemup2.quantity = 10000;
                         itemup3.quantity = 10000;
                         itemup4.quantity = 10000;
                         itemup5.quantity = 10000;
                         
                    player.c.addItemBag(true, itemup); 
                    player.c.addItemBag(true, itemup1);
                    player.c.addItemBag(true, itemup2);
                    player.c.addItemBag(true, itemup3); 
                    player.c.addItemBag(true, itemup4);
                    player.c.addItemBag(true, itemup5);
                }
...
//                if (GameSrc.mapNotPK(p.c.tileMap.map.id) && p.c.typepk > 0) {
...
//                        p.conn.sendMessageLog("Không thể bán quá nhiều!");
//                        return;
//                    }
                    ...
                    if (ItemTemplate.isTypeBody(item.id) && item.upgrade > 0) {
                        p.conn.sendMessageLog("Không thể bán trang bị đã nâng cấp");
                    } else {
                        ItemTemplate data = ItemTemplate.ItemTemplateId(item.id);
                        if (ItemTemplate.isTypeNgocKham(item.id)) {
                    ...
                    if (n.tradeLock == 2) {
                        ...
                        for (i = 0; i < n.tradeIdItem.size(); ++i) {
                            ...
                            if (item3 != null) {
                                ...
                                item1.add(item3);
                                if (item3.quantity > 30000) {
                                    Service.ServerMessage(player.c, "Giao dịch thất bại, số lượng item giao dịch không được vượt quá 30k");
                                    return;
                                }
    ...
    public static void changeTypePk(Player player, Message m) {
        try {
            if (player != null && player.c != null && player.conn != null && m != null && m.reader().available() > 0) {
                ...
                if (GameSrc.mapNotPK(player.c.tileMap.map.id)) {
                    ...
                    for (i = 0; i < player.sortNinja.length; ++i) {
                        if (name.equals(player.sortNinja[i])) {
                            ...
                            if (player.c != null) {
                                ...
                                m = new Message(-23);
                                m.writer().writeInt(player.c.get().id);
                                m.writer().writeUTF("chào mừng bạn đã đăng nhập sever blow do ADMIN quản lý và vận hành. \n Anh em chơi game vui giữ gìn sức khỏe, mọi thắc mắc liện hệ zalo ADMIN");
                                Calendar rightNow;
                                int hour;
                                int min;
                                int sec;
                                        ...
                                        if (ninjaBot.get().level >= 90) {
                                            ...
                                            for (int i2 = 67; i2 <= 72; ++i2) {
                                                ...
                                                if (ski != null) {
                                                    ...
                                                    ninjaBot.clone.open(System.currentTimeMillis() + 60000 * ninjaBot.get().getPramSkill(i2), (int) ninjaBot.get().getPramSkill(71));
                                                    break;
                                                }
                                            }
                                        }
