...
public class GameSrc {
    ...
    public static int[] randomOption = new int[]{70, 71, 72, 80, 81, 82, 83, 86, 87, 88, 89, 90, 91, 92};
    public static int[] randomParam = new int[]{100, 100, 100, 100, 100, 2000, 2000, 100, 2000, 2000, 2000, 2000, 100, 100};
    public static int[] tileXuatHien = new int[]{60, 60, 60, 75, 70, 65, 60, 55, 50, 45, 40, 35, 30, 25};
    public static int[] tilenang = new int[]{100, 80, 70, 60, 50, 40, 30, 20, 10, 10, 7, 7, 5, 5, 2, 2};
    ...
    public static int[] coinGotngoc = new int[]{0, 5000, 40000, 135000, 320000, 625000, 1080000, 1715000, 2560000, 3645000, 5000000, 10000000};
    //exptutien
    ...
    public static int[] percentUpBK = new int[]{80, 65, 50, 40, 30, 25, 20, 15, 8, 5};
    ...
    public static void NangMat(Player p, Item item, int type) throws IOException {
        ...
        if (p.c.quantityItemyTotal(695 + item.upgrade) < 10) {
            ItemTemplate data = ItemTemplate.ItemTemplateId(695 + item.upgrade);
        ...
        try {
            ...
            if (type == 34 && num > 0) {
                ...
                if (clan == null) {
                    ...
                    if ((sell.id >= 423 && sell.id <= 954) || sell.id == 281) {
    ...
    public static byte PickClass(byte nclass) {
        switch (nclass) {
            ...
            default: {
                if (nclass == 6 || nclass == 5|| nclass == 8) {
                    return 3;
                }
                if (nclass == 4 || nclass == 3|| nclass == 8) {
                    return 2;
                }
                if (nclass == 2 || nclass == 1|| nclass == 8) {
                    return 1;
                }
                return 0;
            }
    ...
    public static byte SideClass(byte nclass) {
        if (nclass == 4 || nclass == 6|| nclass == 8) {
            return 1;
        } else if (nclass == 2|| nclass == 8) {
            return 2;
        } else if (nclass == 3|| nclass == 8) {
            return 3;
        } else if (nclass == 7|| nclass == 8) {
            return 4;
        } else {
            return 0;
        }
    ...
    public static void doConvertUpgrade(Player p, Message m) {
        try {
            ...
            if (item1 != null && item2 != null && item3 != null) {
                ...
                if (data1.type >= 10) {
                    p.conn.sendMessageLog("Không thể chuyển hóa vật phẩm này nhé!");
                    return;
                }
                item1.isLock = true;
                item2.isLock = true;
    ...
    public static void LuckValue(Player p, Message m) {
        try {
            ...
            if (p.c.nhiemvu == 2) {
                p.c.latthe++;
                if (p.c.latthe == 1000) {
                    p.sendAddchatYellow("Bạn đã hoàn thành nhiệm vụ tháng 10.");
                }
            }
    ...
    public static void LuyenThach(Player p, Message m) throws IOException, SQLException {
        byte[] arrItem = new byte[m.reader().available()];
    ...
    public static void TinhLuyen(Player p, Message m) {
        try {
            ...
            if (tl == -1) {
                p.conn.sendMessageLog("Lỗi hiển thị vật phẩm hệ thống tự động out!");
                int TimeSeconds = 2;
                while (TimeSeconds > 0) {
                    TimeSeconds--;
                    Thread.sleep(1000);
    ...
    public static void DichChuyen(Player p, Message m) throws IOException, SQLException {
        byte index = m.reader().readByte();
        Item item = p.c.getIndexBag(index);
        ItemTemplate data = ItemTemplate.ItemTemplateId(item.id);
        if (item != null && ItemTemplate.isTypeBody(item.id) && item.upgrade > 11) {
            ...
            if (data.type >= 10) {
                p.conn.sendMessageLog("Đéo cho dịch chuyển vật phẩm này nhá!");
                return;
            }
            if (data.id == 795 || data.id == 796 || data.id == 799 || data.id == 800) {
                p.conn.sendMessageLog("Đéo cho dịch chuyển vật phẩm này nhá!");
                return;
            }
    ...
    public static void luyenNgoc(Player p, Message m) throws IOException, SQLException {
        byte index = m.reader().readByte();
        switch ((int) index) {
            //khảm
            case 0: {
...
//                        yenThaoNgoc = itemThaoNgoc.options.get(i).param;
    ...
    private static void handleUpgradeMat(Player p, Item item, int type) {
        try {
            ...
            p.c.removeItemBags(695 + item.upgrade, 10);
