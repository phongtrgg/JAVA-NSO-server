...
import LTD.stream.Dun;
import LTD.stream.Admin;
import LTD.stream.GiaTocChien;
...
//skgiaikhac
                case 252: {
                    ...
                    if (p.c.quantityItemyTotal(924) >= 5 * soluong && p.c.quantityItemyTotal(929) >= 5 * soluong && p.c.quantityItemyTotal(928) >= 3 * soluong) {
                        ...
                        if (p.c.getBagNull() == 0) {
                            ...
                            p.c.removeItemBags(924, (int) (5 * soluong));
                            ...
                            Item it = ItemTemplate.itemDefault(927);
                            it.quantity = (int) (1 * soluong);
                            p.c.addItemBag(true, it);
                        }
                        return;
                ...
                case 253: {
                    ...
                    if (p.c.quantityItemyTotal(925) >= 500 * soluong && p.c.quantityItemyTotal(929) >= 300 * soluong && p.c.quantityItemyTotal(928) >= 300 * soluong) {
                        ...
                        if (p.c.getBagNull() == 0) {
                            ...
                            Item it = ItemTemplate.itemDefault(926);
                            it.quantity = (int) (1 * soluong);
                            p.c.addItemBag(true, it);
                        }
                        return;
                ...
                case 254: {
                    String check = str.replaceAll("\\s+", "");
                    if (!Util.isNumericInt(str) || check.equals("") || !Util.isNumericInt(str)) {
                        Service.chatNPC(p, (short) 35, "Giá trị nhập vào không đúng");
                        break;
                    }
                    ...
                    if (p.c.quantityItemyTotal(480) >= 5 * soluong && p.c.quantityItemyTotal(481) >= 5 * soluong && p.c.quantityItemyTotal(482) >= 5 * soluong && p.c.quantityItemyTotal(484) >= 4 * soluong) {
                        if (p.c.yen < 50000 * soluong) {
                        ...
                        if (p.c.getBagNull() == 0) {
                            ...
                            p.c.removeItemBags(480, (int) (5 * soluong));
                            p.c.removeItemBags(481, (int) (5 * soluong));
                            p.c.removeItemBags(482, (int) (5 * soluong));
                            p.c.removeItemBags(484, (int) (4 * soluong));
                            p.c.upyenMessage(-(50000 * soluong));
                            p.upluongMessage(-(100 * soluong));
                            p.c.upxuMessage(-(50000 * soluong));
                            Item it = ItemTemplate.itemDefault(827);
                            it.quantity = (int) (1 * soluong);
                            p.c.addItemBag(true, it);
                            p.c.topgiaikhac += (int) (1 * soluong);
                            Service.chatNPC(p, (short) 35, "Thành công bạn nhận được hộp quà");
                        }
                        return;
                    } else {
                        Service.chatNPC(p, (short) 35, "Không đủ nguyên liệu trong hành trang");
                    }
                    break;
                }
                case 255: {
                    String check = str.replaceAll("\\s+", "");
                    if (!Util.isNumericInt(str) || check.equals("") || !Util.isNumericInt(str)) {
                        Service.chatNPC(p, (short) 56, "Giá trị nhập vào không đúng");
                        break;
                    }
                    ...
                    if (p.c.quantityItemyTotal(236) >= 100 * soluong && p.c.quantityItemyTotal(788) >= 5) {
                        if (p.luong < 50 * soluong) {
                            p.conn.sendMessageLog("Không đủ lượng");
                            return;
                        }
                        if (p.c.getBagNull() == 0) {
                            ...
                            p.c.removeItemBags(236, (int) (100 * soluong));
                            p.c.removeItemBags(788, (int) (5 * soluong));
                            p.upluongMessage(-(50 * soluong));
                            Item it = ItemTemplate.itemDefault(1005);
                            it.quantity = (int) (1 * soluong);
                            p.c.addItemBag(true, it);
                            Service.chatNPC(p, (short) 56, "Thành công bạn nhận được đan dược chân hỏa");
                        }
                        return;
                    } else {
                        Service.chatNPC(p, (short) 56, "Không đủ nguyên liệu trong hành trang");
                    }
                    break;
                }
                case 1406: {
                ...
                case 110: {
                    ...
                    if (p.c.quantityItemyTotal(638) >= 3 * soluong && p.c.quantityItemyTotal(639) >= 5 * soluong && p.c.quantityItemyTotal(640) >= 1 * soluong && p.c.quantityItemyTotal(641) >= 3 * soluong && p.c.quantityItemyTotal(642) >= 2 * soluong) {
                        ...
                        if (p.c.getBagNull() == 0) {
                            ...
                            p.c.removeItemBags(638, (int) (5 * soluong));
                            p.c.removeItemBags(639, (int) (5 * soluong));
                            p.c.removeItemBags(640, (int) (5 * soluong));
                            p.c.removeItemBags(641, (int) (5 * soluong));
                            p.c.removeItemBags(642, (int) (5 * soluong));
                            p.c.upyenMessage(-(50000 * soluong));
                            p.c.upxuMessage(-(50000 * soluong));
                            p.upluongMessage(-(100 * soluong));
                ...
                case 111: {
                    ...
                    if (p.c.quantityItemyTotal(638) >= 5 * soluong && p.c.quantityItemyTotal(639) >= 5 * soluong && p.c.quantityItemyTotal(641) >= 5 * soluong && p.c.quantityItemyTotal(642) >= 3 * soluong) {
                        ...
                        if (p.c.getBagNull() == 0) {
                            ...
                            p.c.removeItemBags(639, (int) (5 * soluong));
                            p.c.removeItemBags(641, (int) (5 * soluong));
                            p.c.removeItemBags(642, (int) (3 * soluong));
                ...
                case 41_0:
                    if (p.check) {
                        ...
                        p.sendAddchatYellow("Phát hiện bug BAN ACC VV");
                        int TimeSeconds = 2;
                        while (TimeSeconds > 0) {
                            TimeSeconds--;
                            Thread.sleep(1000);
                ...
                case 12: {
                    ...
                    try {
                        ...
                        if (red != null && red.first()) {
                            ...
                            if (coinchange <= 0) {
                                p.conn.sendMessageLog("Có cái nịt mà bug");
                                p.lockAcc();
                                return;
                            }
                            if (coinchange <= coinold) {
                                coinold -= coinchange;
                                if (coinchange >= 40000) {
                                    if ((p.c.xu + 50000 * coinchange) > 2000000000L) {
                                        p.sendAddchatYellow("Số xu sau khi đổi quá 2.000.000.000 không thể đổi");
                                        return;
                                    }
                                    p.c.upxuMessage(50000 * coinchange);
                                } else {
                                    if ((p.c.xu + 50000 * coinchange) > 2000000000L) {
                                        p.sendAddchatYellow("Số xu sau khi đổi quá 2.000.000.000 không thể đổi");
                                        return;
                    ...
                    try {
                        ...
                        if (red != null && red.first()) {
                            ...
                            if (coinchange <= coinold) {
                                coinold -= coinchange;
                                if (coinchange >= 20000) {
                                    if ((p.c.yen + 100000 * coinchange) > 2000000000L) {
                                        p.sendAddchatYellow("Số yên sau khi đổi quá 2.000.000.000 không thể đổi");
                                        return;
                                    }
                                    p.c.upyenMessage(100000 * coinchange);
                                } else {
                                    if ((p.c.yen + 100000 * coinchange) > 2000000000L) {
                                        p.sendAddchatYellow("Số yên sau khi đổi quá 2.000.000.000 không thể đổi");
                                        return;
                                ...
                                p.conn.sendMessageLog("Bạn không đủ coin để đổi ra yên.");
                            }
                            p.flush();
                            red.close();
                        }
