...
import LTD.template.ItemOptionTemplate;
...
import LTD.real.ClanMember;
import LTD.real.Waypoint;
import LTD.real.Npc;
...
import LTD.real.Alert;
import LTD.real.Map;
import LTD.cache.Part;
import LTD.cache.PartImage;
import LTD.cache.SkillInfoPaint;
import LTD.cache.SkillOptionTemplates;
import LTD.cache.SkillPaint;
import LTD.io.Message;
import LTD.io.SQLManager;
import LTD.io.Util;
import LTD.stream.Client;
import LTD.thiendiabang.ThienDiaData;
import LTD.cache.ArrowPaint;
import LTD.cache.EffectCharPaint;
import LTD.cache.EffectInfoPaint;
...
public class Manager {
    ...
    public static int runtimeconnect=0;
    ...
    public Manager() {
        ...
        this.event = 5;
        this.NinjaS = new String[]{"Ninja Vô Danh", "Ninja Kiếm", "Ninja Phi Tiêu", "Ninja Kunai", "Ninja Cung", "Ninja Đao", "Ninja Quạt", "Ninja Hắc Ám", "Ninja Đại Đế" ,"Ninja Lôi Diện"};
        taixiu = new TaiXiu[1];
        this.taixiu[0] = new TaiXiu();
        chanle = new ChanLEonl[1];
        this.chanle[0] = new ChanLEonl();
        this.loadConfigFile();
        this.rotationluck[0] = new Lucky("Vòng xoay vip", (byte) 0, (short) 120, 1000000, 50000000, 2000000000);
    ...
    private void loadConfigFile() {
        ...
        if (configMap.containsKey("version-Event")) {
            ...
            this.event = (byte) Util.nextInt(1,8);
        } else {
            this.event = 5;
        }
    ...
    private void loadDataBase() {
        ...
        try {
            ...
            res.close();
            System.out.println("LTD.server.Manager.loadDataBase(nj_arrow) COMPLETE");
            i = 0;
            ...
            res.close();
            System.out.println("LTD.server.Manager.loadDataBase(nj_image) COMPLETE");
            i = 0;
            ...
            res.close();
            System.out.println("LTD.server.Manager.loadDataBase(nj_effect) COMPLETE");
            i = 0;
            ...
            res.close();
            System.out.println("LTD.server.Manager.loadDataBase(nj_skill) COMPLETE");
            i = 0;
    ...
    public void sendSkill(Player p) throws IOException {
        ...
        try {
            ...
            m.writer().writeByte(10);//nclasssize
            ...
            for (short j = 0; j < SkillTemplate.entrys.size(); ++j) {
                if (SkillTemplate.entrys.get(j).nclass == 7) {
                    m.writer().writeByte(SkillTemplate.entrys.get(j).id);
                    m.writer().writeUTF(SkillTemplate.entrys.get(j).name);
                    m.writer().writeByte(SkillTemplate.entrys.get(j).maxPoint);
                    m.writer().writeByte(SkillTemplate.entrys.get(j).type);
            ...
            m.writer().writeUTF("Ninja Đại Đế");
            m.writer().writeByte(10);
            for (short j = 0; j < SkillTemplate.entrys.size(); ++j) {
                if (SkillTemplate.entrys.get(j).nclass == 8) {
                    m.writer().writeByte(SkillTemplate.entrys.get(j).id);
                    m.writer().writeUTF(SkillTemplate.entrys.get(j).name);
                    m.writer().writeByte(SkillTemplate.entrys.get(j).maxPoint);
                    m.writer().writeByte(SkillTemplate.entrys.get(j).type);
            ...
            m.writer().writeUTF("Ninja Lôi Diện");
            m.writer().writeByte(4);
            for (short j = 0; j < SkillTemplate.entrys.size(); ++j) {
                if (SkillTemplate.entrys.get(j).nclass == 9) {
                    m.writer().writeByte(SkillTemplate.entrys.get(j).id);
                    m.writer().writeUTF(SkillTemplate.entrys.get(j).name);
                    m.writer().writeByte(SkillTemplate.entrys.get(j).maxPoint);
                    m.writer().writeByte(SkillTemplate.entrys.get(j).type);
